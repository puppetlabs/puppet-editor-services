...
describe 'validation_queue' do
  ...
  describe '#enqueue' do
    ...
    before(:each) do
      ...
      allow(PuppetLanguageServer::Manifest::DocumentValidator).to receive(:validate_epp).and_raise("PuppetLanguageServer::Manifest::DocumentValidator.validate_epp mock should not be called")
    end
    ...
    context 'for a multiple items in the queue' do
      ...
      it 'should only return the most recent validation results' do
        ...
        expect(PuppetLanguageServer::Manifest::DocumentValidator).to receive(:validate_epp).with(file_content1, workspace).and_return(validation_result)
    ...
    context 'for a single item in the queue' do
      ...
      context 'of a EPP template file' do
        ...
        before(:each) do
          expect(PuppetLanguageServer::Manifest::DocumentValidator).to receive(:validate_epp).with(FILE_CONTENT, workspace).and_return(validation_result)
        end
