...
describe 'message_router' do
  ...
  describe '#receive_request' do
    ...
    context 'given a textDocument/onTypeFormatting request' do
      ...
      context 'without the hashrocket featureflag' do
        before(:each) do
          allow(PuppetLanguageServer).to receive(:featureflag?).with('hashrocket').and_return(false)
        end
      ...
      context 'with the hashrocket featureflag' do
        before(:each) do
